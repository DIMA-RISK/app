...
-- =====================
-- ADDENDUM 15: Audit fixes (Option C compliance gap, invite expiry, GDPR target date)
-- Run this block in the Supabase SQL Editor.
-- =====================

-- 15a. GDPR configurable compliance target date (replaces the spreadsheet's
-- hardcoded "25 May 2018") + org-level risk appetite tolerance override.
ALTER TABLE organizations
  ADD COLUMN IF NOT EXISTS gdpr_target_date DATE;

-- 15b. Invite expiry + inviter tracking (EWNAF spec Section 1 fields).
ALTER TABLE beta_codes
  ADD COLUMN IF NOT EXISTS expires_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS invited_by UUID;

-- Backfill existing unused codes with a 14-day window from creation.
UPDATE beta_codes SET expires_at = created_at + INTERVAL '14 days'
  WHERE expires_at IS NULL AND used_at IS NULL;

-- Updated invite function: sets a 14-day expiry on each new code.
CREATE OR REPLACE FUNCTION invite_beta_user(p_email TEXT)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO beta_codes (code, email, expires_at)
  VALUES (v_code, lower(trim(p_email)), now() + INTERVAL '14 days');
...
-- 15c. Option C Compliance Gap: 4-framework average (ISO + HIPAA + PIPEDA + GDPR) / 4.
-- The assigned framework (pipeda or hipaa) uses its full questionnaire gap (richest
-- signal); the other three use their 5-control critical-controls checklist gap.
-- EWNAF technical blend (60/40) still wraps the averaged gap.
CREATE OR REPLACE FUNCTION calculate_risk_score(p_session_id UUID)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
  v_user_id          UUID;
...
BEGIN
  ...
  -- Component 4: Compliance Gap — 4-framework average (Option C)
  SELECT
    COUNT(*) FILTER (WHERE r.response IN ('no', 'partial')),
    COUNT(*) FILTER (WHERE r.response != 'na')
  INTO v_failed, v_total
...
-- =====================
-- ADDENDUM 16: per-org risk-appetite tolerance override (EWNAF spec 2.1).
-- Defaults to $250K; editable per organization from the Risk Register page.
-- Run this block in the Supabase SQL Editor.
-- =====================
ALTER TABLE organizations
...
-- =====================
-- ADDENDUM 20: remove the postal `address` field entirely.
-- The registration form no longer collects it, but organizations.address was
-- still NOT NULL, so new sign-ups failed with:
--   null value in column "address" of relation "organizations" violates not-null constraint
-- Dropping the column removes the field (and its constraint) for good. This does
-- NOT touch org_ip ("Network IP Address"), which is a separate, still-used field.
-- Run this block in the Supabase SQL Editor.
-- =====================
ALTER TABLE organizations DROP COLUMN IF EXISTS address;

-- Refresh PostgREST's schema cache so the dropped column stops being expected.
NOTIFY pgrst, 'reload schema';


-- =====================
-- ADDENDUM 21: fix Regulatory Fine Avoidance blowing up the ROI section.
-- The prior version used revenue-percentage / real-enforcement tiers (Quebec
-- Law 25 up to C$25M, GDPR €20M≈$30M), producing an ~$30M "worst-case fine" and
-- an implausible 1000%+ ROI. Per EWNAF_Risk_Module_Spec §4.3 the fine ceiling is
-- a FLAT, capped total — HIPAA $1.5M + PIPEDA $100K + GDPR $2M overlay ≈ $3.6M
-- worst case — scaled by the non-compliance gap. Everything else (breach cost,
-- ROI investment/benefit math) is unchanged. Run in the Supabase SQL Editor.
-- =====================
CREATE OR REPLACE FUNCTION calculate_financial_impact(p_session_id UUID)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
...
-- Recompute existing sessions so the corrected fines/ROI take effect immediately.
DO $$
DECLARE s RECORD;
BEGIN
...
-- =====================
-- ADDENDUM 24: GDPR overlay fine applies ONLY when GDPR actually applies.
-- Previously the $2M GDPR overlay was added to EVERY org's regulatory fine, so a
-- PIPEDA-only org showed ~$2M instead of its $100K cap. Now the overlay is added
-- only when the org processes EU-resident data (new organizations.processes_eu_data
-- flag, set in onboarding) or is directly assigned the GDPR framework. Everything
-- else is unchanged from ADDENDUM 22. Run this block in the Supabase SQL Editor.
-- =====================
ALTER TABLE organizations ADD COLUMN IF NOT EXISTS processes_eu_data BOOLEAN DEFAULT false;
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(o.has_financial_data, false),
    COALESCE(o.vendor_data_share_pct, 0),
    COALESCE(o.processes_eu_data, false),
    o.annual_revenue,
    o.employee_count,
    o.business_size
  INTO v_records, v_has_health, v_has_financial, v_share_pct, v_eu_data,
       v_revenue, v_employees, v_business_size
  FROM organizations o WHERE o.user_id = v_user_id;
  ...
  -- Assigned-framework flat fine tier, scaled by the non-compliance gap.
  IF v_framework_id = 'hipaa' THEN
    v_fine_max := 1500000 * (v_gap_pct / 100.0);
  ELSIF v_framework_id = 'pipeda' THEN
    v_fine_max := 100000 * (v_gap_pct / 100.0);
  ...
  -- GDPR overlay — ONLY when GDPR actually applies (org processes EU-resident
  -- data, or GDPR is the assigned framework). Otherwise no GDPR fine at all.
  SELECT
    (COUNT(*) FILTER (WHERE NOT COALESCE(r.present, false)))::NUMERIC
    / NULLIF(COUNT(*), 0) * 100
  INTO v_gdpr_gap
  ...
  IF v_eu_data OR v_framework_id = 'gdpr' THEN
    v_gdpr_fine := 2000000 * (v_gdpr_gap / 100.0);
  ELSE
    v_gdpr_fine := 0;
  END IF;
  ...
  v_fine_max := v_fine_max + v_gdpr_fine;
  v_fine_min := ROUND(v_fine_max * 0.4, 2);

  -- ROI projection (unchanged; Security Training still bounded to ≤2% revenue).
  IF v_revenue IS NOT NULL THEN
    SELECT COALESCE(rs.total_score, 0) INTO v_risk_score
    FROM risk_scores rs WHERE rs.session_id = p_session_id;
...
-- Recompute existing sessions so the gated GDPR fine takes effect immediately.
DO $$
DECLARE s RECORD;
BEGIN
  FOR s IN SELECT id FROM assessment_sessions LOOP
...
-- =====================
-- ADDENDUM 25: Risk Register adopts the 26-item healthcare risk catalog.
-- The category column had a CHECK limiting it to 6 generic values
-- (operational/financial/…). The CEO chose the 17 healthcare control categories
-- from the risk-register workbook (Access Control, Data Protection, Governance, …)
-- and wants the catalog extensible over time — so drop the CHECK and validate the
-- category in the app instead. Existing rows keep their stored value.
-- Run this block in the Supabase SQL Editor.
-- =====================
ALTER TABLE risk_register_entries DROP CONSTRAINT IF EXISTS risk_register_entries_category_check;
...
-- =====================
-- ADDENDUM 26: Mandatory override flag on risk entries (CEO ROI-transparency spec).
-- A mandatory (legally-required) control is recommended for implementation
-- regardless of ROI — ROI is shown for information only. Non-mandatory controls
-- follow the ROI threshold (proceed vs defer). Default false; set per entry (and,
-- later, defaulted from the risk catalog once the CEO supplies Mandatory Y/N).
-- Run this block in the Supabase SQL Editor.
-- =====================
ALTER TABLE risk_register_entries ADD COLUMN IF NOT EXISTS mandatory BOOLEAN DEFAULT false;
